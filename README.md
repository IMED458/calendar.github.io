  ...
  <script>
    ...
    function renderDoctorList() {
      ...
      filtered.forEach(doc => {
        ...
        if (selectedDoctor && selectedDoctor.name === doc.name && selectedDoctor.phone === doc.phone) {
    ...
    addFinalBtn.addEventListener('click', () => {
      ...
      const finalHours = repeat === 'daily' ? '8' : hours;
    ...
    function renderCalendar() {
      ...
      dayNames.forEach(d => {
        const el = document.createElement('div'); el.className = 'day-name'; el.textContent = d;
    ...
    function selectDate(date) {
      selectedDate = date;
      selectedDateTitle.textContent = formatDateDDMMYYYY(date); // აქ გამოიყენება ახალი ფორმატი
    ...
    exportBtn.onclick = () => {
      if (!selectedDate) return alert('აირჩიეთ თარიღი');
      ...
      XLSX.writeFile(wb, `მორიგეები_${formatDateDDMMYYYY(selectedDate)}.xlsx`); // Excel-შიც DD/MM/YYYY
    };
