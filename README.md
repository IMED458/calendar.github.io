...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .day-cell.today {
      background: #ef4444 !important;   /* წითელი ფონი */
    ...
    .day-cell.has-shift {
      background: #ecfdf5; border-color: var(--accent);
      box-shadow: 0 0 8px rgba(16,185,129,0.3);
    }
    .day-cell .date-num { font-weight: 600; margin-bottom: 3px; }
    .day-cell .shift-count {
      font-size: 10px; color: var(--accent); font-weight: bold; position: absolute; bottom: 4px; right: 6px;
    }
    ...
    .shift-hours {
      background: #ecfdf5; color: #059669; padding: 3px 8px; border-radius: 6px;
      font-weight: 600; font-size: 11px;
    }
    .delete-btn {
      background: var(--danger); color: white; border: none; padding: 3px 7px;
      border-radius: 5px; font-size: 10px; cursor: pointer;
    }

    .modal { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%;
      background: rgba(0,0,0,0.5); z-index: 1000; justify-content: center; align-items: center; padding: 15px;
    }
...
<body>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div style="margin-top:20px;padding-top:16px;border-top:1px solid var(--gray);">
          ...
          <div class="form-group"><input type="text" id="new-specialty" placeholder="სპეციალობა" /></div>
  ...
  <script>
    ...
    doctorsRef.on('value', snapshot => {
      ...
      doctors = data ? Object.keys(data).map(k => ({ id: k, ...data[k] })) : [];

      // თუ ცარიელია – ჩავსვათ დეფაულტები
      if (doctors.length === 0) {
        const defaults = [
        ...
        defaults.forEach(d => doctorsRef.push().set(d));
        doctors = defaults.map((d, i) => ({ id: 'temp' + i, ...d }));
      }

      populateSpecialties();
      renderDoctorList();
      updateStatus('ექიმები სინქრონიზებული', '#10b981');
    }, err => {
      console.error(err);
      updateStatus('ექიმების შეცდომა', '#ef4444');
    });

    // ==================== Shifts (Firebase) ====================
    shiftsRef.on('value', snapshot => {
      const data = snapshot.val();
      allShifts = data ? Object.keys(data).map(k => ({ id: k, ...data[k] })) : [];
      renderCalendar();
      if (selectedDate) renderShiftsForDate(selectedDate);
    });

    function saveShift(shift) {
      shiftsRef.push().set(shift);
    }
    ...
    function renderDoctorList() {
      const term = doctorSearch.value.toLowerCase().trim();
      const spec = specialtyFilter.value;
      const filtered = doctors.filter(d =>
        d.name.toLowerCase().includes(term) && (!spec || d.specialty === spec)
      );
      doctorList.innerHTML = filtered.length === 0
        ? '<div style="padding:15px;text-align:center;color:#666;">ექიმი არ მოიძებნა</div>'
        : '';
      filtered.forEach(doc => {
        const div = document.createElement('div');
        div.className = 'doctor-item';
        if (selectedDoctor && selectedDoctor.id === doc.id) div.classList.add('selected');
        div.innerHTML = `<strong>${doc.name}</strong><br><small>${doc.specialty} • ${doc.phone}</small>`;
        div.onclick = () => {
          selectedDoctor = doc;
    ...
    function renderCalendar() {
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (dayShifts.length) {
          ...
          cell.innerHTML = `<div class="date-num">${day}</div><div class="shift-count">${dayShifts.length}</div>`;
        } else {
    ...
    function renderShiftsForDate(date) {
      ...
      const byDept = {};
      shifts.forEach(s => {
        (byDept[s.specialty] = byDept[s.specialty] || []).push(s);
      });
      departmentsGrid.innerHTML = Object.keys(byDept).length === 0
        ? '<div style="text-align:center;color:#666;padding:20px;">მორიგეობა არ არის</div>'
        : '';
      ...
      document.querySelectorAll('.delete-btn').forEach(btn => {
        btn.onclick = () => {
          if (confirm('დარწმუნებული ხართ?')) deleteShift(btn.dataset.id);
        };
      });
    }

    function filterDepartments() {
      const term = deptSearch.value.toLowerCase().trim();
      document.querySelectorAll('.dept-card').forEach(card => {
        const header = card.querySelector('.dept-header').textContent.toLowerCase();
        card.style.display = header.includes(term) ? 'block' : 'none';
      });
    }
    ...
    exportBtn.onclick = () => {
      ...
      const monthShifts = allShifts.filter(s => s.date.startsWith(`${y}-${m}`));
      const byDept = {};

      monthShifts.forEach(s => {
        const dept = s.specialty;
        const doc = s.doctor;
        if (!byDept[dept]) byDept[dept] = {};
        if (!byDept[dept][doc]) byDept[dept][doc] = { phone: s.phone, hours: {}, count: {} };
        const day = s.date.split('-')[2];
        byDept[dept][doc].hours[day] = (byDept[dept][doc].hours[day] || 0) + parseInt(s.hours);
        byDept[dept][doc].count[day] = (byDept[dept][doc].count[day] || 0) + 1;
      });

      Object.keys(byDept).forEach(dept => {
        const rows = [['ექიმი','ტელეფონი', ...Array.from({length: daysInMonth}, (_,i)=>i+1), 'ჯამი საათები','ჯამი მორიგეობები']];
        Object.keys(byDept[dept]).forEach(doc => {
          const d = byDept[dept][doc];
          const row = [doc, d.phone];
          let totH = 0, totC = 0;
          for (let day=1; day<=daysInMonth; day++) {
            const h = d.hours[day] || 0;
            const c = d.count[day] || 0;
            row.push(h || '');
            totH += h; totC += c;
          }
          row.push(totH, totC);
      ...
      const monthName = ['იანვარი','თებერვალი','მარტი','აპრილი','მაისი','ივნისი','ივლისი','აგვისტო','სექტემბერი','ოქტომბერი','ნოემბერი','დეკემბერი'][parseInt(m)-1];
      XLSX.writeFile(wb, `მორიგეები_${monthName}_${y}.xlsx`);
    };
    ...
    addFinalBtn.onclick = () => {
      ...
      const base = new Date(modalDate.value);
      const hours = modalHours.value;
      const repeat = repeatType.value;
      const until = parseInt(repeatUntil.value) || 30;
      const dates = [];

      if (repeat === 'none') dates.push(modalDate.value);
      else if (repeat === 'daily') {
        let cur = new Date(base);
        for (let i = 0; i <= until; i++) {
          const wd = cur.getDay();
          if (wd >= 1 && wd <= 5) dates.push(cur.toISOString().split('T')[0]);
    ...
    document.getElementById('add-new-doctor').onclick = () => {
      ...
      if (name && spec && phone) {
        doctorsRef.push().set({ name, specialty: spec, phone }).then(() => {
          alert('ექიმი დაემატა');
